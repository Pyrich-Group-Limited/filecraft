...
namespace DocumentManagement.API.Helpers
{
    public class JobService
    {
        ...
        public void StartScheduler()
        {
            ...
            //Reminder Scheduler To Send Email
            RecurringJob.AddOrUpdate("DocumentIndexing", () => DocumentIndexing(), "*/5 * * * *", new RecurringJobOptions { TimeZone = TimeZoneInfo.Local }); // Every 10 minutes
        ...
        public bool SendEmailScheduler()
        {
        ...
        public bool DocumentIndexing()
        {
            var schedulerStatus = _connectionMappingRepository.GetDocumentIndexingStatus();
            if (!schedulerStatus)
            {
                _connectionMappingRepository.SetDocumentIndexingStatus(true);
                var result = _mediator.Send(new AddDocumentIndexContentCommand()).GetAwaiter().GetResult();
                _connectionMappingRepository.SetDocumentIndexingStatus(false);
                return result;
            }
            return true;
        }
